...
# setup.sh — Universal setup script for vex-tools
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/vex-tools/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
main() {
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/vex-tools.git"
        info "Then: cd vex-tools && ./setup.sh"
        exit 1
    fi
