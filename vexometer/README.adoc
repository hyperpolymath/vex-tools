...
A rigorous, reproducible tool for quantifying the irritation surface of LLMs and related tools, producing standardised metrics that complement existing benchmarks (MMLU, HumanEval, MT-Bench) with human experience dimensions.
...
ISA produces an *Irritation Surface Analysis (ISA)* score from 0-100, where *lower is better*. The score aggregates ten measurable dimensions of user experience degradation.
...
# Run benchmarks (JSON + HTML radar reports + markdown by default)
just bench
...
== Benchmarking and Dogfooding

ISA now includes a reproducible benchmark harness intended for local dogfooding runs.
...
# Faster iteration pass
just bench-quick
----

Artifacts are written to:

* `benchmarks/results/latest.json` - machine-readable benchmark summary
* `benchmarks/results/<timestamp>.json` - immutable run snapshot
* `benchmarks/results/cases/*.html` - per-case analysis reports with radar chart + ISA gauge
* `benchmarks/results/cases/*.{json,md,...}` - additional display formats

Display format selection is controlled by:

* `VEXOMETER_BENCH_DISPLAY_FORMATS` (default: `html,json,md`)
* accepted values: `html,json,md,csv,latex,yaml`

Database backend for benchmark persistence defaults to *VeriSimDB*:

* `VEXOMETER_BENCH_DB_BACKEND=verisimdb`
* repo-local profile file: `config/verisim-profile.env`
* `VEXOMETER_BENCH_VERISIM_MODE=auto` starts a repo-local sidecar on an auto-selected port (default range `18080-18999`)
* `VEXOMETER_BENCH_VERISIM_MODE=external` uses `VEXOMETER_BENCH_VERISIM_EXTERNAL_URL`
* `VEXOMETER_BENCH_VERISIM_MODE=disabled` skips publish
* explicit endpoint override: `VERISIM_API_URL`
* optional local core checkout hint: `VERISIM_HOME`
* set `VEXOMETER_BENCH_REQUIRE_DB=1` to fail the run if publish fails

The profile is runtime-only: it does not vendor the VeriSimDB codebase or any database data into this repository.

== API Providers

ISA prioritises local/open models for privacy and reproducibility:
...
----
+-----------------------------------------------------------------------+
|  ISA (Vexometer) - Irritation Surface Analyser                [-][o][x]|
+-----------------------------------------------------------------------+
| +---------------+ +---------------------+ +-----------------------+ |
| | Model: [v    ]| |                     | | Findings              | |
| +---------------+ |    /\   TII: 2.3    | +-----------------------+ |
...
ISA is a *diagnostic instrument*—it measures irritation surfaces but does not fix them. Interventions that reduce irritation are implemented in separate *satellite repositories*.
...
Contributions welcome under MPL-2.0. See link:CONTRIBUTING.adoc[CONTRIBUTING.adoc].
