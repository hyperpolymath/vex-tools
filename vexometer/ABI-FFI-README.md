...
# VEXOMETER ABI/FFI Documentation
...
│       └── include/
│           └── vexometer.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── vexometer.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/vexometer.h
```
...
int main() {
    void* handle = vexometer_init();
    if (!handle) return 1;

    int result = vexometer_process(handle, 42);
    if (result != 0) {
        const char* err = vexometer_last_error();
...
Compile with:
```bash
gcc -o example example.c -lvexometer -L./zig-out/lib
```

### From Idris2

```idris
import VEXOMETER.ABI.Foreign
...
#[link(name = "vexometer")]
extern "C" {
    fn vexometer_init() -> *mut std::ffi::c_void;
    fn vexometer_free(handle: *mut std::ffi::c_void);
    fn vexometer_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = vexometer_init();
        assert!(!handle.is_null());

        let result = vexometer_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libvexometer = "libvexometer"

function init()
    handle = ccall((:vexometer_init, libvexometer), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:vexometer_process, libvexometer), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:vexometer_free, libvexometer), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/vexometer.h
   ```
