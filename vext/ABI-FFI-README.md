...
│       └── include/
│           └── vext.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── vext.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/vext.h
```
...
int main() {
    void* handle = vext_init();
    if (!handle) return 1;

    int result = vext_process(handle, 42);
    if (result != 0) {
        const char* err = vext_last_error();
...
Compile with:
```bash
gcc -o example example.c -lvext -L./zig-out/lib
```

### From Idris2

```idris
import VEXT.ABI.Foreign
...
#[link(name = "vext")]
extern "C" {
    fn vext_init() -> *mut std::ffi::c_void;
    fn vext_free(handle: *mut std::ffi::c_void);
    fn vext_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = vext_init();
        assert!(!handle.is_null());

        let result = vext_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libvext = "libvext"

function init()
    handle = ccall((:vext_init, libvext), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:vext_process, libvext), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:vext_free, libvext), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/vext.h
   ```
