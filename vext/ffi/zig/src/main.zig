...
const VERSION = "0.1.0";
const BUILD_INFO = "VEXT built with Zig " ++ @import("builtin").zig_version_string;
...
export fn vext_free(handle: ?*Handle) void {
...
export fn vext_process(handle: ?*Handle, input: u32) Result {
...
export fn vext_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn vext_free_string(str: ?[*:0]const u8) void {
...
export fn vext_last_error() ?[*:0]const u8 {
...
export fn vext_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn vext_build_info() [*:0]const u8 {
...
/// Register a callback
export fn vext_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn vext_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(vext_is_initialized(handle) == 1);
}

test "error handling" {
    const result = vext_process(null, 0);
