...
# LAZY_ELIMINATOR ABI/FFI Documentation
...
│  generated/abi/lazy-eliminator.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to liblazy-eliminator.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── lazy-eliminator.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── lazy-eliminator.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/lazy-eliminator.h
```
...
```c
#include "lazy-eliminator.h"

int main() {
    void* handle = lazy-eliminator_init();
    if (!handle) return 1;

    int result = lazy-eliminator_process(handle, 42);
    if (result != 0) {
        const char* err = lazy-eliminator_last_error();
...
Compile with:
```bash
gcc -o example example.c -llazy-eliminator -L./zig-out/lib
```

### From Idris2

```idris
import LAZY_ELIMINATOR.ABI.Foreign
...
```rust
#[link(name = "lazy-eliminator")]
extern "C" {
    fn lazy-eliminator_init() -> *mut std::ffi::c_void;
    fn lazy-eliminator_free(handle: *mut std::ffi::c_void);
    fn lazy-eliminator_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = lazy-eliminator_init();
        assert!(!handle.is_null());

        let result = lazy-eliminator_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const liblazy-eliminator = "liblazy-eliminator"

function init()
    handle = ccall((:lazy-eliminator_init, liblazy-eliminator), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:lazy-eliminator_process, liblazy-eliminator), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:lazy-eliminator_free, liblazy-eliminator), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/lazy-eliminator.h
   ```
