...
# SATELLITE_TEMPLATE ABI/FFI Documentation
...
│  generated/abi/satellite-template.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libsatellite-template.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── satellite-template.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── satellite-template.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/satellite-template.h
```
...
```c
#include "satellite-template.h"

int main() {
    void* handle = satellite-template_init();
    if (!handle) return 1;

    int result = satellite-template_process(handle, 42);
    if (result != 0) {
        const char* err = satellite-template_last_error();
...
Compile with:
```bash
gcc -o example example.c -lsatellite-template -L./zig-out/lib
```

### From Idris2

```idris
import SATELLITE_TEMPLATE.ABI.Foreign
...
```rust
#[link(name = "satellite-template")]
extern "C" {
    fn satellite-template_init() -> *mut std::ffi::c_void;
    fn satellite-template_free(handle: *mut std::ffi::c_void);
    fn satellite-template_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = satellite-template_init();
        assert!(!handle.is_null());

        let result = satellite-template_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libsatellite-template = "libsatellite-template"

function init()
    handle = ccall((:satellite-template_init, libsatellite-template), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:satellite-template_process, libsatellite-template), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:satellite-template_free, libsatellite-template), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/satellite-template.h
   ```
